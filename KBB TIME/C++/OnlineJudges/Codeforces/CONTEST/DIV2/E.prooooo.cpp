...
int n;
vector<int> dpp[MAXN], dpp2[MAXN];
pair<int,int> edges[MAXN];

int par1[MAXN], subtree1[MAXN], tin[MAXN], tout[MAXN], diffArr[MAXN], smallCount[MAXN];
int timer1;
...
JAV()
{
    ...
    while (t--) {
        ...
        while(!stack3.empty()){
            int v = stack3.back();
            if (idx3[v] < (int)dpp2[v].size()) {
                int u = dpp2[v][idx3[v]++];
                if (u == parent3[v]) continue;
                parent3[u] = v;
                stack3.push_back(u);
            } else {
                stack3.pop_back();
        ...
        while(pq.size() > 1){
            ...
            int v1 = groups[i1].back(); groups[i1].pop_back();
            int v2 = groups[i2].back(); groups[i2].pop_back();
